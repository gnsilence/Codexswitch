...
public sealed class CsUsageTrendChart : Control
{
    ...
    public static readonly StyledProperty<IEnumerable<IUsageTrendChartPoint>?> ItemsSourceProperty =
        AvaloniaProperty.Register<CsUsageTrendChart, IEnumerable<IUsageTrendChartPoint>?>(nameof(ItemsSource));

    public static readonly StyledProperty<UsageTrendChartGranularity> GranularityProperty =
        AvaloniaProperty.Register<CsUsageTrendChart, UsageTrendChartGranularity>(
            nameof(Granularity),
    ...
    private IUsageTrendChartPoint[] _items = [];
    ...
    static CsUsageTrendChart()
    {
        ...
        ItemsSourceProperty.Changed.AddClassHandler<CsUsageTrendChart>((chart, args) =>
            chart.OnItemsSourceChanged(args.OldValue as IEnumerable<IUsageTrendChartPoint>, args.NewValue as IEnumerable<IUsageTrendChartPoint>));
    ...
    public IEnumerable<IUsageTrendChartPoint>? ItemsSource
    {
        get => GetValue(ItemsSourceProperty);
        set => SetValue(ItemsSourceProperty, value);
    }

    public UsageTrendChartGranularity Granularity
    {
    ...
    private void OnItemsSourceChanged(IEnumerable<IUsageTrendChartPoint>? oldValue, IEnumerable<IUsageTrendChartPoint>? newValue)
    {
    ...
    private void EnsureDataCache()
    {
        ...
        {
            null => [],
            IUsageTrendChartPoint[] array => array,
            ICollection<IUsageTrendChartPoint> collection => ToArray(collection),
            IReadOnlyCollection<IUsageTrendChartPoint> collection => ToArray(collection),
    ...
    private void RebuildAxisLabels()
    {
        ...
        for (var index = 0; index <= 4; index++)
        {
            ...
            _leftAxisLabels[index] = CreateTextLayout(UsageChartValueFormatter.FormatTokenCount(value), 11, AxisBrush, TextAlignment.Right);
        }

        _rightAxisLabels = _costMax > 0m
            ? [
                CreateTextLayout(UsageChartValueFormatter.FormatCost(_costMax), 11, AxisBrush),
                CreateTextLayout(UsageChartValueFormatter.FormatCost(0m), 11, AxisBrush)
            ]
            : [];
    ...
    private void DrawTooltip(DrawingContext context, Rect plot, Point anchor, IUsageTrendChartPoint item, long totalTokens)
    {
        ...
        var cacheHitRate = UsageChartValueFormatter.CalculateCacheHitRate(
            item.InputTokens,
            item.CachedInputTokens,
            item.CacheCreationInputTokens);
        var outputTps = UsageChartValueFormatter.CalculateOutputTokensPerSecond(item.OutputTokens, item.OutputDurationMs);
        ...
        DrawText(context, $"{TokensLabel}: {UsageChartValueFormatter.FormatTokenCount(totalTokens)}", new Point(left + 12, top + 32), 11, TooltipMutedBrush);
        DrawText(context, $"{RequestsLabel}: {item.Requests:N0}", new Point(left + 12, top + 50), 11, TooltipMutedBrush);
        DrawText(context, $"{CostLabel}: {UsageChartValueFormatter.FormatCost(item.Cost)}", new Point(left + 12, top + 68), 11, TooltipMutedBrush);
        DrawText(
            context,
            $"{CacheHitRateLabel}: {UsageChartValueFormatter.FormatPercentage(cacheHitRate)}",
            new Point(left + 12, top + 86),
            11,
            TooltipMutedBrush);
        DrawText(
            context,
            $"{OutputTpsLabel}: {UsageChartValueFormatter.FormatTokensPerSecond(outputTps)}",
        ...
        long value)
    {
        ...
        DrawText(
            context,
            $"{label}: {UsageChartValueFormatter.FormatTokenCount(value)}",
            new Point(x + 13, y),
            10.5,
            BreakdownTextBrush);
    }
    ...
    {
        if (Granularity == UsageTrendChartGranularity.Day)
    ...
    private static ChartSeries CreateSeries(
        string name,
        string hexColor,
        Func<IUsageTrendChartPoint, long> valueSelector)
    {
    ...
    private static IUsageTrendChartPoint[] ToArray(ICollection<IUsageTrendChartPoint> collection)
    {
        var array = new IUsageTrendChartPoint[collection.Count];
        collection.CopyTo(array, 0);
        return array;
    }

    private static IUsageTrendChartPoint[] ToArray(IReadOnlyCollection<IUsageTrendChartPoint> collection)
    {
        var array = new IUsageTrendChartPoint[collection.Count];
        var index = 0;
